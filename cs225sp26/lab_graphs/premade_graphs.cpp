...
Graph PremadeGraphs::createChinaMap(bool isWeighted)
{
        ...
        vector<Vertex> vertices(num);
        for (size_t i = 0; i < num; ++i) {
                vertices[i] = graph.insertVertex(labels[i]);
                graph.setVertexName(i, labels[i]);
        }

        graph.insertEdge(vertices[2], vertices[7]); /* Beijing - Wuhan */
        graph.insertEdge(vertices[2], vertices[8]); /* Beijing - Nanjing */
        graph.insertEdge(vertices[8], vertices[1]); /* Nanjing - Shanghai */
        graph.insertEdge(vertices[1], vertices[0]); /* Shanghai - Hangzhou */
        graph.insertEdge(vertices[0], vertices[7]); /* Hangzhou - Wuhan */
        graph.insertEdge(vertices[7], vertices[8]); /* Wuhan - Nanjing */
        graph.insertEdge(vertices[7], vertices[4]); /* Wuhan - Guangzhou */
        graph.insertEdge(vertices[4], vertices[3]); /* Guangzhou - Shenzhen */
        graph.insertEdge(vertices[3], vertices[5]); /* Shenzhen - Hongkong */
        graph.insertEdge(vertices[1], vertices[6]); /* Shanghai - Taiwan */
        graph.insertEdge(vertices[4], vertices[6]); /* Guangzhou - Taiwan */
        graph.insertEdge(vertices[9], vertices[7]); /* Chongqing - Wuhan */
        graph.insertEdge(vertices[0], vertices[4]); /* Hangzhou - Guangzhou */
        graph.insertEdge(vertices[2], vertices[9]); /* Beijing - Chongqing */
        graph.insertEdge(vertices[2], vertices[1]); /* Beijing - Shanghai */

        if (isWeighted) {
                graph.setEdgeWeight(vertices[2], vertices[7], 1142); 	/* Beijing - Wuhan */
                graph.setEdgeWeight(vertices[2], vertices[8], 995);  	/* Beijing - Nanjing */
                graph.setEdgeWeight(vertices[8], vertices[1], 297);  	/* Nanjing - Shanghai */
                graph.setEdgeWeight(vertices[1], vertices[0], 173);  	/* Shanghai - Hangzhou */
                graph.setEdgeWeight(vertices[0], vertices[7], 694);  	/* Hangzhou - Wuhan */
                graph.setEdgeWeight(vertices[7], vertices[8], 530);  	/* Wuhan - Nanjing */
                graph.setEdgeWeight(vertices[7], vertices[4], 989);  	/* Wuhan - Guangzhou */
                graph.setEdgeWeight(vertices[4], vertices[3], 130);  	/* Guangzhou - Shenzhen */
                graph.setEdgeWeight(vertices[3], vertices[5], 45);   	/* Shenzhen - Hongkong */
                graph.setEdgeWeight(vertices[1], vertices[6], 1052);  	/* Shanghai - Taiwan */
                graph.setEdgeWeight(vertices[4], vertices[6], 1092);  	/* Guangzhou - Taiwan */
                graph.setEdgeWeight(vertices[9], vertices[7], 929);  	/* Chongqing - Wuhan */
                graph.setEdgeWeight(vertices[0], vertices[4], 1249); 	/* Hangzhou - Guangzhou */
                graph.setEdgeWeight(vertices[2], vertices[9], 1761); 	/* Beijing - Chongqing */
                graph.setEdgeWeight(vertices[2], vertices[1], 1221); 	/* Beijing - Shanghai */
        }
