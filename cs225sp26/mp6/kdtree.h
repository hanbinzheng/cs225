...
private:
    ...
    static inline bool shouldReplace(const Point<Dim> &curr_best, 
				     const Point<Dim> &potential, 
				     double best_dist_sq, double dist_pot) {
		if (dist_pot < best_dist_sq) {
			return true;
		} else if (dist_pot == best_dist_sq) {
			return (potential < curr_best);
		} else {
			return false;
		}	
	}
public:
  ...
  private:
    ...
    KDTreeNode *copy(KDTreeNode const * const subroot);
    void clear(KDTreeNode *subroot);
    void clear(void);
    void copy(KDTree const &other);
    // int partition(vector<Point<Dim>>& v, int idx_l, int idx_r, int idx_pivot, int dim);
    // void QuickSelect(vector<Point<Dim>>& v, int idx_l, int idx_r, int idx_k, int dim);
    void findNearestNeighbor(KDTreeNode *subroot, 
	const Point<Dim> &query, int dim, Point<Dim> &curr_best, double &best_dist_sq) const;
    KDTreeNode *BuildFromVector(
	std::vector<Point<Dim>> &v, int idx_l, int idx_r, int dim);

    /* helper dist function */
    static inline double dist_sq(const Point<Dim> &a, const Point<Dim> &b) {
	double dist = 0.0f;
	#if (Dim==3) /* specific version */
		#if defined(__APPLE__) && defined(__clang__) /* apple clang */
			double dx = a[0] - b[0];
			double dy = a[1] - b[1];
			double dz = a[2] - b[2];
