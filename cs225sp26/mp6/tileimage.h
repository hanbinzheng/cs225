/**
 * tileimage.h (v2)
 * Definition of the TileImage class.
 */

#ifndef TILEIMAGE_H
#define TILEIMAGE_H

#include <math.h>
#include <stdint.h>
#include <string>
#include "cs225/PNG.h"
#include "cs225/HSLAPixel.h"
#include "point.h"

using namespace cs225;

/**
 * Represents a Tile in the Photomosaic.
 */
class TileImage {
private:
	std::string filename_;
    	int cropped_resolution_;
    	HSLAPixel averageColor_;

public:
    	TileImage() : averageColor_(HSLAPixel()) {}
    	explicit TileImage(const std::string& filename);
    	HSLAPixel getAverageColor() const { return averageColor_; }
    	int getResolution() const { return cropped_resolution_; }
	std::string getFileName() const { return filename_; }

	/* class util functions, resized should be of the correct size */	
 	PNG cropSourceImage(const PNG& source);
	/* this function will set part of canvas to be resized, start from (x, y) */
    	void paste(PNG& canvas, int startX, int startY, const PNG &resized) const;
	/* this function will resize the cropped PNG to resized */
	void generateResizedImage(int cropped_resolution, const PNG &cropped, PNG &resized) const;

private:
 	HSLAPixel calculateAverageColor(const PNG &image) const;
    	HSLAPixel getScaledPixelDouble(double startX, double endX,
                                double startY, double endY, const PNG &cropped) const;
    	HSLAPixel getScaledPixelInt(int startX, int endX,
                                int startY, int endY, const PNG &cropped) const;

    	/* basic number calculation */
    	static uint64_t divide(uint64_t a, uint64_t b) {
      		return (a + b / 2) / b;
    	}
    	static int divide(int a, int b) {
        	return divide(static_cast<uint64_t>(a), static_cast<uint64_t>(b));
    	}
    	static int fdivide(double a, double b) {
        	return a / b + 0.5;
    	}
    	static double frac(double x) {
    		return x - floor(x);
    	}
};

#endif // TILEIMAGE_H
