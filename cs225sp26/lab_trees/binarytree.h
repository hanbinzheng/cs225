...
template <typename T> class BinaryTree {
  ...
  private:
    ...
    /* helper function for public mirror() */
    void mirror(Node *&sub_root);

    /* helper function for public printPaths() */
    void get_all_paths(vector<vector<T>> &paths, vector<T> &curr_path,
                       Node *sub_root) const;

    /* helper function for sunDistances() */
    int sum_distance(Node *sub_root, int sub_root_depth) const;

    void in_order(Node *sub_root, vector<T> &tree_vector) const;
};
