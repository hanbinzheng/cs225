...
void BTree<K, V>::split_child(BTreeNode* parent, size_t child_idx)
{
    ...
    /* handle parents */
    parent->elements.insert(elem_itr, *mid_elem_itr); /* promote mdeian to parents */
    parent->children.insert(child_itr, new_right); /* link new right */

    if (!child->is_leaf) { /* not leaf, split the child */
...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
	...
	if (subroot->is_leaf) { /* leaf case */
		if (first_larger_idx < subroot->elements.size() && 
			subroot->elements[first_larger_idx] == pair.key) {
