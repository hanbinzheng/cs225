    ...
    : backingImage(image), resolution(setResolution)
{
    ...
    rows = getRows_();
    cols = getColumns_();
    region_color.resize(rows * cols); /* resize + index, instead of push_back */

    for (int x = 0; x < rows; x++) {
	for (int y = 0; y < cols; y++) {
	    region_color[x * cols + y] = regionColorProcess(x, y);
	}
    }
}
...
{
	return region_color[row * cols + col];
}

HSLAPixel SourceImage::regionColorProcess(int row, int col) {
...
int SourceImage::getRows() const { return rows; }
int SourceImage::getColumns() const { return cols; }
