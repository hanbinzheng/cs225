...
void HuffmanTree::buildTree(const vector<Frequency> &frequencies) {
    ...
    while (singleQueue.size() + mergeQueue.size() > 1) {
        TreeNode *min1 = removeSmallest(singleQueue, mergeQueue);
        TreeNode *min2 = removeSmallest(singleQueue, mergeQueue);
        TreeNode *new_node = new TreeNode((min1->freq).getFrequency() +
                                          (min2->freq).getFrequency());
        new_node->left = min1;
        new_node->right = min2;
...
void HuffmanTree::decode(stringstream &ss, BinaryFileReader &bfile) {
    ...
    while (bfile.hasBits()) {
        ...
        /* huffman tree has either no child node, or 2 childs */
...
void HuffmanTree::writeTree(TreeNode *current, BinaryFileWriter &bfile) {
    ...
    if (current->left == nullptr && current->right == nullptr) {
        char byte = (current->freq).getCharacter();
        bfile.writeBit(true);
...
HuffmanTree::TreeNode *HuffmanTree::readTree(BinaryFileReader &bfile) {
     ...
     */
    TreeNode *subtree_root = nullptr;
    if (bfile.hasBits()) {
        bool bit = bfile.getNextBit();
        if (bit) {
            char byte = bfile.getNextByte();
            subtree_root = new TreeNode(Frequency(byte, 0));
        } else {
            subtree_root = new TreeNode(Frequency('\0', 0));
            subtree_root->left = readTree(bfile);
            subtree_root->right = readTree(bfile);
        }
    }
    return subtree_root;
}
